...
function looksTransient(error: unknown) {
  ...
  return message.includes('failed to fetch') || message.includes('network') || message.includes('load failed') || message.includes('timeout') || message.includes('fetch');
}
...
export async function addAiPlayer(code: string) {
  const { data, error } = await supabase.rpc('add_ai_player', { p_code: normalizeRoomCode(code) });
...
export async function removeAiPlayer(code: string, playerId: string) {
  const { error } = await supabase.rpc('remove_ai_player', { p_code: normalizeRoomCode(code), p_player_id: playerId });
  if (error) throw new Error(errorToMessage(error, 'تعذر إزالة لاعب AI'));
}

export async function castAiVotes(code: string) {
  const { data, error } = await supabase.rpc('cast_ai_votes', { p_code: normalizeRoomCode(code) });
...
export async function revealNextRound(code: string) {
  const { error } = await supabase.rpc('reveal_next_round', { p_code: normalizeRoomCode(code) });
...
export async function restartDiscussionTimer(code: string, seconds: number) {
  const { error } = await supabase.rpc('restart_discussion_timer', { p_code: normalizeRoomCode(code), p_seconds: seconds });
  if (error) throw new Error(errorToMessage(error, 'تعذر إعادة العداد'));
}

export async function rematchRoom(code: string) {
  const { error } = await supabase.rpc('reset_room_for_rematch', { p_code: normalizeRoomCode(code) });
...
function apiUrl(path: string) {
  ...
  if (!base) throw new Error('حط EXPO_PUBLIC_APP_URL علشان تطبيق الموبايل يوصل لسيرفر اللعبة');
  return `${base}${path}`;
}
...
export function shareRoomUrl(code: string) {
  ...
  if (Platform.OS === 'web' && typeof window !== 'undefined') return `${window.location.origin}/room/${normalizeRoomCode(code)}`;
