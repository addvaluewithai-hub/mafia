-- Populate the descriptive case role when a case supplies one, without requiring a fictional character name.
-- Legacy name/bio fields remain supported until the generator/story schema migrates in a later session.
...
begin
  ...
  loop
    ...
    set character_name = nullif(trim(v_character->>'name'), ''),
        ...
        case_role = nullif(trim(v_character->>'role'), ''),
        is_eliminated = false
